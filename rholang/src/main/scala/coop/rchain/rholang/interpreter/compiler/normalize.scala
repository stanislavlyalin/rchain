...
object ProcNormalizeMatcher {
  ...
  /**
    ...
    */
  def normalizeMatch[F[_]: Sync](p: Proc, input: ProcVisitInputs): F[ProcVisitOutputs] =
    Sync[F].defer {
      ...
      def binaryExp[T](
        ...
        for {
          ...
          input.par
            .prepend(constructor(leftResult.par, rightResult.par), input.boundMapChain.depth),
          rightResult.freeMap
        )
